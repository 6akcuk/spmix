...
?>
<tr>
    <td><?php echo ActiveHtml::checkBox('select['. $purchase->purchase_id .']') ?></td>
    <td><?php echo $purchase->purchase_id ?></td>
    <td><?php echo ActiveHtml::date($purchase->create_date, true, true) ?></td>
    ...
    <td><?php echo ActiveHtml::link(Yii::t('purchase', '{n} заказ|{n} заказа|{n} заказов', $purchase->ordersNum), '/orders'. $purchase->purchase_id) ?></td>
    <td></td>
</tr>
<?php endforeach; ?>